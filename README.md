...
| **🛡️ 智能防骚扰** | **短 ID 机制**修复了 Telegram 按钮点击失效的 Bug。普通验证通过后提供 **7 天免打扰期**，`/trust` 用户永久免验证；验证前发送的消息不会自动转发，普通用户前 3 条正常消息禁止链接和 `@用户名`，降低广告首条直达风险。 |
...
7.  **⚠️ 关键步骤：绑定数据库与变量**
    ...
    * **添加环境变量**：
        ...
        * 可选反广告变量：`BLOCKED_DOMAINS`、`ALLOWED_DOMAINS`、`BLOCKED_USERNAMES`、`ALLOWED_USERNAMES`，多个值可用逗号或空格分隔。
